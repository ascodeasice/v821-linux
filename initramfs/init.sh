...
# (1) absolute: rdcycle/rdtime, falls back to a fixed-instruction loop if rdcycle traps.
# A27 clock 的 mux 設定 + 一次相對速度量測。絕對值以 felcpux 印的暫存器推導為準
# （HOSC 40MHz x N/D），這裡的 iters/s 只用來跨次比較，精度約 ±7%。
